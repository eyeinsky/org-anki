...
Requires [[https://foosoft.net/projects/anki-connect/][AnkiConnect]] to be added to [[https://apps.ankiweb.net/][Anki]].[fn:via]

Features:
- sync simple org entries to Anki, entry's title is taken as =Front=
  (=Name=) and content an =Back= (=Descr=)
- note updates
- sync/update tags
- tag inheritance from global and file level properties, and from ancestors
- custom deck name defined globally, per file, per subtree, or even for each
  org entry separately
- Cloze notes via interactive prompt (a al anki-editor) or via
  ={{..}}= syntax directly
- sync single org entry (convenient to bind this to a keyboard shortcut)
- customize Anki note type and field names
- import external deck from Anki (requires pandoc for html-to-org conversion).
...
** Why
.. as there are [[https://github.com/louietan/anki-editor][anki-editor]], [[https://github.com/abo-abo/pamparam][pamparam]], [[https://gitlab.com/phillord/org-drill][org-drill]], [[https://github.com/l3kn/org-fc][org-fc]], [[https://github.com/c-okelly/org_to_anki][org_to_anki]]?

This project started out to support converting simple org entries to
Anki notes where note `Front` field is taken from the title and `Back`
field is taken from the content. Over time many more features have
been added.
