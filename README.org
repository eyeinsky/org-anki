...
Tags are also synchronized; hierarchical tags are enabled by
customizing =org-anki-hierarchical-tags-separator= to a string that
serves as hierarchy level separator. This is converted to-and-from
=::= when syncing entries.
...
- =org-anki-upload-images= :: set to =t= to sync file content from
  file links to Anki as well. Default is =nil=.
